...
# ── UNVAN SEÇENEKLERİ ────────
# Format: { "Görünen Ad": (aranacak_kelimeler, hariç_tutulacaklar) }
unvan_secenekleri = {
    "Tümü (Toplam)":       ([], []),
...
with st.sidebar:
    ...
    if st.button("🔄 Tüm Filtreleri Temizle", use_container_width=True):
        for k in ["ay_key", "pm_key", "bm_key", "hrbp_key", "segment_key"]:
            default = "Tümü (Nisan & Mayıs)" if k == "ay_key" else "Tümü"
            st.session_state[k] = default
        for k in ["ay_select", "pm_select", "bm_select", "hrbp_select", "segment_select"]:
            default = "Tümü (Nisan & Mayıs)" if k == "ay_select" else "Tümü"
            st.session_state[k] = default
        st.rerun()
    ...
    segment_idx = segment_listesi.index(st.session_state.segment_key) if st.session_state.segment_key in segment_listesi else 0
    st.markdown("🏷️ **Segment**")
    sec_segment = st.selectbox("Segment", segment_listesi, index=segment_idx, key="segment_select", label_visibility="collapsed")
...
with sekme1:
    ...
    if goster_mayis:
        ...
        with k4: st.metric("📅 2025 Nisan TO", f"%{kpi25:.1f}")
        with k5: st.metric("📆 2025 Mayıs TO MTD", f"%{kpi_may25:.1f}")
        with k6: st.metric("📊 2025 YTD Toplam TO", f"%{kpi_ytd25:.1f}")
    else:
        k4, k5 = st.columns(2)
        with k4: st.metric(f"📅 {ana_baslik25}", f"%{kpi25:.1f}")
        with k5: st.metric("📊 2025 YTD Toplam TO", f"%{kpi_ytd25:.1f}")
    ...
    if not df_segment_to.empty:
        ...
        with seg_chart_col:
            ay_label = sec_ay if sec_ay != "Tümü (Nisan & Mayıs)" else "Nisan"
...
with sekme2:
    ...
    st.markdown("Mağaza ve unvan seçimi yaparak dönem içi hareketleri ve turnover oranlarını inceleyebilirsiniz.")
    ...
    if df_detay.empty or magaza_col_detay is None:
        ...
        if sec_magaza_detay:
            try:
                ...
                # Dönem Başı
                haric_genel = ["müdür", "vm", "sorumlu", "satış", "yardım", "yrd", "mmy"]
                if sec_unvan == "Tümü (Toplam)":
                    db_col = (bul_kolon(df_detay.columns, ["dönem başı"], haric_genel) or
                              bul_kolon(df_detay.columns, ["baş", "çalışan"], haric_genel))
                    giris_col_detay  = bul_kolon(df_detay.columns, ["giriş"], haric_genel)
                    cikis_col_detay  = (bul_kolon(df_detay.columns, ["işten çıkış"], haric_genel + ["gönüllü", "gönülsüz", "zorunlu", "oran"]) or
                                        bul_kolon(df_detay.columns, ["çıkış"], haric_genel + ["gönüllü", "gönülsüz", "zorunlu", "oran"]))
                    gonullu_col      = (bul_kolon(df_detay.columns, ["gönüllü", "çıkış"], haric_genel + ["oran"]) or
                                        bul_kolon(df_detay.columns, ["gönüllü"], haric_genel + ["oran"]))
                    gonulsuz_col     = (bul_kolon(df_detay.columns, ["gönülsüz", "çıkış"], haric_genel + ["oran"]) or
                                        bul_kolon(df_detay.columns, ["gönülsüz"], haric_genel + ["oran"]))
                    zorunlu_col      = (bul_kolon(df_detay.columns, ["zorunlu", "çıkış"], haric_genel + ["oran"]) or
                                        bul_kolon(df_detay.columns, ["zorunlu"], haric_genel + ["oran"]))
                    ds_col           = (bul_kolon(df_detay.columns, ["dönem sonu"], haric_genel) or
                                        bul_kolon(df_detay.columns, ["son", "çalışan"], haric_genel))
                else:
                    db_col           = (bul_kolon(df_detay.columns, ["dönem başı"] + keys, exclude) or
                                        bul_kolon(df_detay.columns, ["baş", "çalışan"] + keys, exclude))
                    giris_col_detay  = bul_kolon(df_detay.columns, ["giriş"] + keys, exclude)
                    cikis_col_detay  = (bul_kolon(df_detay.columns, ["işten çıkış"] + keys, exclude + ["gönüllü", "gönülsüz", "zorunlu", "oran"]) or
                                        bul_kolon(df_detay.columns, ["çıkış"] + keys, exclude + ["gönüllü", "gönülsüz", "zorunlu", "oran"]))
                    gonullu_col      = (bul_kolon(df_detay.columns, ["gönüllü", "çıkış"] + keys, exclude + ["oran"]) or
                                        bul_kolon(df_detay.columns, ["gönüllü"] + keys, exclude + ["oran"]))
                    gonulsuz_col     = (bul_kolon(df_detay.columns, ["gönülsüz", "çıkış"] + keys, exclude + ["oran"]) or
                                        bul_kolon(df_detay.columns, ["gönülsüz"] + keys, exclude + ["oran"]))
                    zorunlu_col      = (bul_kolon(df_detay.columns, ["zorunlu", "çıkış"] + keys, exclude + ["oran"]) or
                                        bul_kolon(df_detay.columns, ["zorunlu"] + keys, exclude + ["oran"]))
                    ds_col           = (bul_kolon(df_detay.columns, ["dönem sonu"] + keys, exclude) or
                ...
                style_fn = lambda x: "color:#0d1b3e; font-weight:bold; font-size:15px;"
                if hasattr(df_tablo1.style, "map"):
                    styled_tablo1 = df_tablo1.style.map(style_fn, subset=["Sayı / Değer"])
                else:
                    styled_tablo1 = df_tablo1.style.applymap(style_fn, subset=["Sayı / Değer"])

                # Turnover oranları
                to_tum_col      = (bul_kolon(df_detay.columns, ["tüm", "turnover"]) or
                                   bul_kolon(df_detay.columns, ["turnover"], haric_tutulacaklar=["gönüllü","gönülsüz","zorunlu"]))
                ...
                with t_col1:
                    ...
                    st.dataframe(styled_tablo1, use_container_width=True, hide_index=True, height=290)
                with t_col2:
...
with sekme4:
    ...
    if df_yasal.empty:
        ...
        if sec_magaza_yasal:
            try:
                df_filtered_yasal = df_yasal[df_yasal["Display_Magaza"] == sec_magaza_yasal].copy().reset_index(drop=True)
                ...
                if df_filtered_yasal.empty:
                    ...
                    st.warning(f"⚠️ Toplam **{len(df_filtered_yasal)}** adet geç bildirilen yasal çıkış tespit edildi.")
                    df_yasal_goster = df_filtered_yasal[yasal_cols].rename(columns=rename_dict)
                    style_fn_yasal  = lambda x: "color:#0d1b3e; font-weight:bold; font-size:15px;"
                    if "Fark Gün" in df_yasal_goster.columns:
                        if hasattr(df_yasal_goster.style, "map"):
                            styled_yasal = df_yasal_goster.style.map(style_fn_yasal, subset=["Fark Gün"])
                        else:
                            styled_yasal = df_yasal_goster.style.applymap(style_fn_yasal, subset=["Fark Gün"])
                    else:
                        styled_yasal = df_yasal_goster
                    st.dataframe(styled_yasal, use_container_width=True, hide_index=True)
